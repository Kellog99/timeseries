...
Let $\tau, K\in \mathbb{N}$, respectively the number of lag to forecast and the number of step to look backward. Then the aim of a predictive model is to find a function $F:\mathbf{R}^k\rightarrow\mathbf{R}^{s \times \tau}$ such as:
$$
